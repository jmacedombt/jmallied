...
export default async function ConsultaBidPage() {
  ...
  for (let inicio = 0; ; inicio += LOTE_BUSCA) {
    const { data, error } = await supabase
      .from("bid_pecas")
      .select(
        "id, modelo, part_number, custo_peca_samsung, valor_com_margem, custo_peca_allied, valor_imposto, mao_de_obra, travado, travado_em, cadastrado_manualmente, valor_atualizado_em, valor_direcao, bid_solucoes(id, peca_solucao, principal)"
      )
