...
// Tela "Orçamentos Enviados" (menu Operacional, pedido explícito) —
// MESMO histórico que já existia como pop-up dentro de Validação de
// Orçamentos ("Histórico de Envios", botão "Confirmar Envio"), agora
// também como página própria, inclusive pro login ALLIED (pedido
// explícito — mesmo arquivo que a Allied já recebe pra aprovar os
// orçamentos, nada de custo/BID nessa planilha).
export default function PainelOrcamentosEnviados() {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm" style={{ color: "var(--muted)" }}>
        Toda vez que &quot;Confirmar Envio&quot; é confirmado em Validação de Orçamentos, a planilha final fica
      ...
      {envios && envios.length > 0 && (
        <p className="text-xs flex items-center gap-1.5" style={{ color: "var(--muted)" }}>
          <History size={12} />
          {envios.length} envio(s) no total.
        </p>
      )}
    </div>
  );
